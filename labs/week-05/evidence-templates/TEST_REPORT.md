...
| TC-L5-03 | เปิด `#/requests/new` แล้วกด F5 | หลัง refresh ยังอยู่หน้า New Request ไม่หายและไม่เจอ 404 | รีเฟรชแล้วฟอร์มไม่หายและไม่เจอ 404 | PASS | ⚠️ ยังไม่มีรูปแนบ |
...
| TC-L5-07 | ลบข้อมูลใน LocalStorage แล้วเปิด Dashboard | ระบบตรวจพบข้อมูลหาย ดึง Seed Data กลับมาลงใหม่อัตโนมัติ | ดึง Seed Data กลับมาลงใหม่อัตโนมัติถูกต้อง | PASS | ❌ ไม่มีไฟล์ `storage-auto-seed.png` — ต้องแคปเพิ่ม |
...
| TC-L5-18 | แอบแก้ LocalStorage ให้พัง แล้วกด Refresh | ระบบตรวจจับข้อมูลพัง กู้คืนข้อมูลเริ่มต้นใหม่ เว็บไม่ค้าง | ตรวจจับและกู้คืนข้อมูลถูกต้องตามที่คาด | PASS | ❌ ไม่มีไฟล์ `storage-recovery.png` — ต้องแคปเพิ่ม |
...
| TC-L5-23 | เปิดเว็บใน Incognito แล้วกด Refresh URL Hash | เว็บทำงานปกติในโหมดไม่ระบุตัวตน Refresh ไม่หลุด | ทำงานได้ปกติตามที่คาด | PASS | ❌ ไม่มีไฟล์ `pages-incognito.png` — ต้องแคปเพิ่ม |
