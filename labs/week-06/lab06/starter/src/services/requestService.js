...
export async function updateStatus(id, status) {
  const found = requests.find((r) => r.id === id); // find the id from array

  if(!found) { // if not found
    return null ;
  }

  // if found change status
  found.status = status ;
  
  await persist(); // call persist save files
  return structuredClone(found); // clone return 
}
...
export async function remove(id) {
  ...
  const isDeleted = requests.length < before;
