...
/**
 *   W06-C1 (CP02) · GET /api/requests
 * - อ่าน req.query.status (ถ้ามี) ส่งต่อให้ service.findAll()
 * - ตอบ 200 พร้อมรายการ
 */
export function listRequests(req, res) {
  const { status } = req.query; //query ใข้สำหรับกรองสถานะคำร้อง (ถ้ามี) 
  res.status(200).json(service.findAll({ status })); // ตอบ 200 พร้อมรายการคำร้องทั้งหมด (หรือกรองตาม status ถ้ามี)
}
